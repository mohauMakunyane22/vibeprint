...
const App = () => {
  ...
  const handleFormSubmit = async (data) => {
    ...
    try {
      ...
      const parsed = parseGeminiOutput(resultJson.text, mode);
...
Only include places located in or very near [User's City]. Do not include places outside this city.

4. A list of 3–4 music artists whose sound matches their vibe.
...
Input: ${input}`;
  ...
  const parseGeminiOutput = (text, mode) => {
    ...
    // Dynamically set either music or marketing insights
    let musicArtists = [];
    let marketingInsights = [];

    if (mode === "individual" && parts[3]) {
      musicArtists = parts[3]
        .split(/[\n,]/)
        .map((name) => name.trim())
        .filter(Boolean);
    }

    if (mode === "brand" && parts[3]) {
      marketingInsights = parts[3]
        .split(/\n/)
